...
def register():
    if request.method == 'POST':
        ...
        try:
            ...
            return redirect(url_for('shop'))  # After successful registration, redirect to the shop page
        except Error as e:
            flash(f"Error: {e}")
            conn.rollback()
        finally:
            cursor.close()
            conn.close()
    return render_template('register.html')  # Corrected the template name here
...
def shop():
    ...
    cart_items = get_cart_items()  # Function to fetch cart items from the session or database
    total_price = calculate_total(cart_items)  # Function to calculate total price
...
def cart():
    ...
    cart_items = get_cart_items_from_session_or_db(session['user_id'])  # Fetch cart items
